...
| B3 | 插件生态规划（已暂缓） | 暂缓开发插件系统，聚焦 GUI 和 Mock | 待定 |
...
**插件系统**（暂缓开发） | ~~插件 SDK~~ | ~~标准化的插件开发接口~~ | ~~暂缓~~
| | ~~官方插件~~ | ~~Database、Redis、GraphQL 等~~ | ~~暂缓~~
| | ~~插件市场~~ | ~~未来规划（v3.0+）~~ | ~~暂缓~~
...
| **v2.1.0** | Week 9-12 | 平台化 | GUI 编辑器 MVP + Mock 服务 |
| **v2.2.0（暂定）** | Week 13-16 | ~~生态完善~~ | ~~插件系统~~ + 认证增强 + 报告升级 |
| **v3.0.0** | 待定 | 平台化完成 | GUI 编辑器完整功能 + Mock 服务完整功能 |
